    ...
    public class PhotoAdapter extends PagerAdapter {
        ...
        public View instantiateItem(ViewGroup container, int position) {
            ...
            if (!TextUtils.isEmpty(path) && path.toLowerCase().contains(".gif")) {
                Glide.with(ShowBigImageListActivity.this).asGif().load(path)//
                        .diskCacheStrategy(DiskCacheStrategy.ALL).into(photoView);
            } else {
                Glide.with(ShowBigImageListActivity.this).asBitmap().load(path)//
                        .diskCacheStrategy(DiskCacheStrategy.ALL).into(photoView);
            }
            ...
            ((ViewPager) container).addView(photoView, ViewPager.LayoutParams.MATCH_PARENT, ViewPager.LayoutParams.MATCH_PARENT);
            return photoView;
        }
